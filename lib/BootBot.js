...
class BootBot extends EventEmitter {
  ...
  getUserProfile(userId) {
    ...
    return fetch(url)
      .then(res => res.json())
      .catch(err => this.logger.error(`Error getting user profile: ${err}`));
  }
  ...
  _initWebhook() {
    var parser = bodyParser.json({ verify: this._verifyRequestSignature.bind(this) });
    this.app.get(this.webhook, parser, (req, res) => {
    ...
    this.app.post(this.webhook, parser, (req, res) => {
      this._logRequest(req);
      var data = req.body;
      if (data.object !== 'page') {
        return;
